...
#define TOPIC_SUB_TEMP    "P1_EV/Sensors/Temperature"
#define TOPIC_SUB_LIGHT   "P1_EV/Sensors/Light"
#define TOPIC_PUB_LIGHT_STATE "P1_EV/Sensors/Light/State"
#define TOPIC_PUB_TEMP_STATE "P1_EV/Sensors/Temperature/State"
...
static void mqtt_incoming_data_cb(void *arg, const u8_t *data, u16_t len, u8_t flags)
{
    ...
    {
        int temperatura = atoi(payload);
        int temp_state = temp_motor_app(temperatura);

        if (temp_state == 1)
        {
            const char *msg = "Ventilador encendido";
            publish_message(NULL, TOPIC_PUB_TEMP_STATE, msg);
        }
        else if (temp_state == -1)
        {
            const char *msg = "Ventilador apagado";
            publish_message(NULL, TOPIC_PUB_TEMP_STATE, msg);
        }
    }
    else if (strcmp(topic_incoming, TOPIC_SUB_LIGHT) == 0)
    {
        int luz = atoi(payload);
        int light_state = light_led_app(luz);

        if (light_state == 1)
        {
            const char *msg = "LED encendido";
            publish_message(NULL, TOPIC_PUB_LIGHT_STATE, msg);
        }
        else if (light_state == -1)
        {
            const char *msg = "LED apagado";
            publish_message(NULL, TOPIC_PUB_LIGHT_STATE, msg);
        }
    }
